...
3. Keep historical paper replay validation passing with timestamp-correct higher-timeframe context for every reviewed instrument.
4. Configure a real external webhook receiver for healthwatch alerts.
5. Monitor weekly restore drill logs and audit events.
6. Configure production blog RSS feeds and live Telegram listener credentials.
7. Run live Telegram ingestion after credentials/session are configured.
